if (true) {
    var approveSVG = `<span title="Verified"><svg style="width:24px;height:24px" viewBox="0 0 24 24">
    ...
    var shareSVG = ` <div id="share-button" title="Share" onclick="share()" style="cursor: pointer;"><svg style="width:24px;height:24px" viewBox="0 0 24 24">
...
function loaded() {
    try {
        ...
        var data = JSON.parse(document.getElementsByClassName("ace_content").item(0).innerText);

        var _newhtml = `<span><div class="titlecard"><div id="titletext" style="` + data.titlestyle + `">` + data.titlecard + `</div>` + shareSVG + `</div></span>`;

        for (q = 0; q < data.results.length; q++) {
            _newhtml += `<div class="item" onclick="alert('You can\'t run JS functions here without unloading the editor!")"><span class="title">` + data.results[q].title + "<div class=\"badges\">";
